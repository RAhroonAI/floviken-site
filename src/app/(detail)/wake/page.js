...
export default function Wake() {
  return (
    <article>
      ...
      <p style={paragraph}>
        <strong style={sectionLabel}>Hypothesis.</strong> The clinician who worked the night is the one who knows what happened on it, so that clinician should produce the handoff, not reconstruct it later from a list that has already moved on. Wake is the overnight layer of that handoff: it reads the night&apos;s departures directly from the chart, lets the clinician annotate them and track any open loops, and folds the result into the morning assignment sheet that the day team already reads. One person produces it; the lock hands it over.
      </p>
      <p style={paragraph}>
        <strong style={sectionLabel}>Method.</strong> Wake adds nothing the chart does not already record. A clinician starts a night shift. Every patient discharged during the shift (to hospice, AMA, a transfer, a routine discharge, or expired) appears automatically as a departure, read from the chart&apos;s own discharge events and scoped to the shift&apos;s window. The clinician can add a short note to any of them for the day team. Departures are derived, never re-entered.
      </p>
      <p style={paragraph}>
        The one thing the chart cannot otherwise carry is an open loop on a patient who is already gone. Wake stores exactly that: a follow-up thread, a note and a status (open, reached, unable to reach, handed off) on an already-discharged patient, opened by the night clinician and closed by whoever resolves it. It is the only clinical content Wake originates from nothing. The shift boundary and the optional departure note are its only other writes, and none of them touch the chart.
      </p>
      <p style={paragraph}>
        <strong style={sectionLabel}>Generate and lock.</strong> At the end of the night, the clinician ends the shift, and that one act produces the morning sheet: it runs the assignment algorithm and freezes the overnight section (departures and open follow-ups) into a single working draft. The clinician reviews it, adjusts an assignment if someone is sicker than the load model knows, then locks it. Locking stamps the sheet with the time and the author and makes it read-only; the lock is the handoff. The day team reads the locked sheet and picks up patients as they assume care. The structured sheet is the handoff. A language model can, on request, render it into prose, but it is never on the path: it writes language only, invents no fact, and the sheet is complete without it.
      </p>
      <p style={paragraph}>
        <strong style={sectionLabel}>Integration.</strong> Wake runs against the synthetic patients in Keel and shares its chart with the morning assignment room. Because departures are read from the same discharge events the worklist and the discharged view already use, the three never disagree about who is on service: a patient who left is absent from the assignments, present in the overnight section, and on the discharged list: one fact, three views. The assignments account for who is here; the overnight layer accounts for everyone who is not.
      </p>
      <p style={paragraph}>
        <strong style={sectionLabel}>Status.</strong> Live. Deployed against the synthetic patients in Keel, producing the morning sheet (assignments plus the overnight layer) as one locked handoff.
      </p>
