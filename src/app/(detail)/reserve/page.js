...
export default function Reserve() {
  return (
    <article>
      ...
      <p style={paragraph}>
        <strong style={sectionLabel}>Background.</strong>{" "}A red blood cell
        spends its whole life holding one variable steady: its redox state.
        Oxidants keep arriving, from the oxygen it carries, from infections, from
        drugs, and each one would damage the cell if left unchecked. The defense
        ...
        arrives that raises the oxidant load, an antimalarial, dapsone,
        rasburicase, even fava beans, and two people on the same dose can diverge:
        one clears it without noticing, the other hemolyzes. The same drug, a
        different outcome. The difference is G6PD activity.
      </p>

      <p style={paragraph}>
        <strong style={sectionLabel}>Hypothesis.</strong>{" "}If you model the
        cell as a control loop with two independent knobs, the loop gain (G6PD
        activity) and the disturbance (the oxidant load), the clinical split
        should fall out without being written in. A high-gain cell rejects the
        disturbance and returns to its setpoint. A low-gain cell looks identical
        at rest, but past a large enough disturbance it cannot make NADPH fast
        enough: the reserve runs down, glutathione collapses behind it, and the
        cell tips into hemolysis. Severity should be a property of the loop, not
        an entry in a lookup table. The deficiency stays silent until the day it
        is challenged.
      </p>
      ...
      <p style={paragraph}>
        <strong style={sectionLabel}>Method.</strong>{" "}A deterministic system
        of rate equations for the coupled redox and glycolytic state (NADPH and
        NADP+, the glutathione couple, ATP, 2,3-BPG, and a cumulative damage
        marker), integrated with a fixed-step Runge-Kutta solver. The G6PD step
        uses the published competitive product- and effector-inhibition rate law
        of Shimo, Nishino &amp; Tomita (2011), driven by their per-patient
        parameters. The surrounding enzymes and the absolute concentration scales
        are reduced and illustrative, flagged as such in the source, pending the
        paper&apos;s full supplement. Damage keys off the NADPH reserve, not the
        glutathione that drains visibly ahead of it. There is no randomness:
        identical inputs produce an identical trajectory, every run.
      </p>

      <p style={paragraph}>
        <strong style={sectionLabel}>Integration.</strong>{" "}The interactive
        page runs that engine in your browser: the same code, no server, no
      ...
      <p style={paragraph}>
        <strong style={sectionLabel}>Why this matters.</strong>{" "}Matching
        drugs to a person&apos;s genetics is already changing how some medicines
        are prescribed, and G6PD deficiency is one of the clearest cases. A single
